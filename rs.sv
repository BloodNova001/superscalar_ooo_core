        ...
        if (reset) begin
            ...
            for (i = 0;i < `DISPATCH_WIDTH ;i = i + 1 ) begin
                if (uops_in[i][`UOP_FU_TYPE] == `FU_ALU) begin
                    ...
                    RS_lsu[tail_lsu] <= {uops_in[i][`UOP_ROB_TAG],uops_in[i][`UOP_LSU_OP],!uops_in[i][`UOP_Q1_VALID],uops_in[i][`UOP_Q1_TAG],value_in[i][31:0],!uops_in[i][`UOP_Q2_VALID],uops_in[i][`UOP_Q2_TAG],value_in[i][63:32]};
                    tail_lsu <= tail_lsu + 1;
                end
            end
        end
